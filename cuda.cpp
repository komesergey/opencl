...
int main(int argc,char *argv[])
{
    ...
    checkCudaErrors(cuLaunchKernel(function, gridSize, 1, 1, blockSize, 1, 1, 0, nullptr, KernelParams, nullptr));
    auto done = std::chrono::high_resolution_clock::now();
    checkCudaErrors(cuMemcpyDtoH(Host_c, devBufferC, N*sizeof(int)));
    auto doneFull = std::chrono::high_resolution_clock::now();
    long resultTime = std::chrono::duration_cast<std::chrono::nanoseconds>(done - start).count();
    long resultTimeFull = std::chrono::duration_cast<std::chrono::milliseconds>(doneFull - startFull).count();
    printf("Time spent adding vector of size %d: %ld nanoseconds\n", N, resultTime);
    printf("Time spent adding vector of size %d: %ld milliseconds (with copying data)\n", N, resultTimeFull);
